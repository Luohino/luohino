...
const NutritionSection = () => {
  ...
  // Simple mobile carousel instead of heavy animations
  const MobileCarousel = () => {
    const [currentIndex, setCurrentIndex] = useState(0);

    return (
      <section className="nutrition-section py-12 md:py-0">
        <div className="flex md:flex-row flex-col justify-between md:px-10 px-5 pt-10 md:pt-0">
        ...
        <div className="overflow-x-auto scrollbar-hide">
          <div className="flex gap-4 px-5 py-8 w-max md:w-full md:justify-center">
            {TESTIMONIALS.map((testimonial, index) => (
              <div
                key={testimonial.id}
                className="flex-shrink-0 w-[90vw] md:w-96 bg-white border-white border-[0.3rem] rounded-2xl p-6 shadow-lg"
              >
                <div className="flex flex-col items-center space-y-4 text-center h-full justify-between">
                  ...
                  <div className="flex-1 flex items-center">
                    <blockquote className="text-[#523122] font-paragraph text-base leading-relaxed">
                      "{testimonial.review}"
                    </blockquote>
                  </div>
    ...
    return (
      <section className="nutrition-section" data-speed="1">
        <ContainerScroll className="h-[140vh]" data-speed="1">
          <div className="sticky left-0 top-0 h-screen w-full flex flex-col will-change-transform">
            {/* Header at top */}
              ...
              <CardsContainer className="h-[450px] w-[90%] max-w-md md:max-w-lg">
                {TESTIMONIALS.map((testimonial, index) => (
                  <CardTransformed
                    ...
                    index={index + 1}
                    variant="light"
                    className="bg-white border-white border-[0.3rem] backdrop-blur-md shadow-lg will-change-transform"
                  >
                    <div className="flex flex-col items-center space-y-4 text-center h-full justify-between">
                      <ReviewStars
                        className="text-yellow-600"
  ...
  return isMobile ? <MobileCarousel /> : <DesktopAnimated />;
};
